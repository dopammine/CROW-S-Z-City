...
net.Receive("brutal_start",function()
	roundend = false
...
hook.Add("Think", "ZoneSoundThink", function()
	if CurrentRound() and CurrentRound().name ~= "brutality" then return end
...
local fighter = {
    objective = "Find weapons to be the sole survivor.",
    name = "Traitor",
    color1 = Color(190,10,10)
}
...
--
    ...
    draw.SimpleText("Homicide | Brutality", "ZB_HomicideMediumLarge", sw * 0.5, sh * 0.1, Color(0,162,255, 255 * fade), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
...
net.Receive("brutal_end",function()
